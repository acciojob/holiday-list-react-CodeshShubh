...
const App = () => {
  ...
  useEffect(() => {
    let filterCountry = cityList.filter((items) => items.country == "India");
    setIndianCity(filterCountry);
  }, []);

  return (
    <div id="main">
      <ol>
        {IndianCity.map((items, index) => {
          return (
            <li key={`location${index + 1}`} style={{ gap: "10px" }}>
              name:{items.name} country:{items.country}
            </li>
          );
        })}
      </ol>
